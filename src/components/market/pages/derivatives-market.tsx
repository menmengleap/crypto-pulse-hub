...
/** Derivatives Market content — shared by the console page (/derivatives) and the public homepage page (/markets/derivatives). */
export function DerivativesMarketContent() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Panel
        ...
        description="Open interest, funding and positioning"
        bodyClassName="p-0"
      >
                ...
                <div key={p.asset.id}>
                  <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2 text-xs">
                    <span className="truncate text-muted-foreground">
                      {p.asset.symbol} perpetual
                    </span>
                    <span className={cn("num", p.funding >= 0 ? "text-up" : "text-down")}>
                      {(p.funding * 100).toFixed(4)}%
                    </span>
                  </div>
                  <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-muted">
                    <div
                      className={cn("h-full rounded-full", p.funding >= 0 ? "bg-up" : "bg-down")}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
