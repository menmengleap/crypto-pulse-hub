...
export const Route = createFileRoute("/spot")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Spot market pairs with live prices, 24h change, volume and trend classification." },
      { property: "og:title", content: "Spot Market — Cryptolytic" },
      { property: "og:description", content: "Track spot pairs, volume and trend across the crypto market." },
    ],
  }),
  component: SpotPage,
});
        ...
        action={
          <div className="flex items-center gap-2">
            <div className="relative hidden sm:block">
              ...
              <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search pair" className="h-8 w-44 pl-8 text-xs" />
            </div>
            ...
            <tbody>
              {rows.map((a) => (
                <tr key={a.id} className="border-b border-border/60 transition-colors last:border-0 hover:bg-accent/40">
                  ...
                  <td className="num px-4 py-3 text-right text-muted-foreground">{fmtCompact(a.marketCap)}</td>
                  <td className="px-4 py-3 text-right"><TrendBadge trend={a.trend} /></td>
                  <td className="px-4 py-3">
                    <Sparkline data={a.spark} positive={a.change7d >= 0} className="ml-auto h-7 w-24" fill={false} />
                  </td>
                </tr>
              ))}
            </tbody>
