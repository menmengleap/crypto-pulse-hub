...
export const Route = createFileRoute("/cycle")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Identify the current crypto market cycle phase with structural and on-chain context." },
    ...
    <AppShell title="Market Cycle" subtitle="Cycle phase and structural context">
      <div className="space-y-5">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard label="Current phase" value="Expansion" hint="72% through phase" accent="primary" />
        ...
        <Panel title="Total market cap" description="Cycle trajectory">
          <Sparkline data={marketCapHistory.map((d) => d.value)} positive className="h-56" strokeWidth={2} />
        </Panel>

        <Panel title="Interpretation">
          <p className="text-sm text-muted-foreground">
            Structure remains expansionary: higher highs on the weekly, dominance grinding upward and breadth still
            narrow. Euphoria conditions — parabolic altcoin breadth, funding above 0.05% and dominance rolling over —
