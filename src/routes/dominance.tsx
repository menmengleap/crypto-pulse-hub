...
export const Route = createFileRoute("/dominance")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Bitcoin and Ethereum dominance trends with historical context and market interpretation." },
      { property: "og:title", content: "Bitcoin Dominance — Cryptolytic" },
      { property: "og:description", content: "Track capital rotation between Bitcoin, Ethereum and altcoins." },
    ],
  }),
    ...
    <AppShell title="Bitcoin Dominance" subtitle="Capital distribution across the market">
      <div className="space-y-5">
        <div className="grid gap-4 sm:grid-cols-3">
          <StatCard label="BTC dominance" value={`${globalStats.btcDominance}%`} change={0.42} accent="btc" />
          <StatCard label="ETH dominance" value={`${globalStats.ethDominance}%`} change={-0.18} accent="primary" />
          <StatCard label="Other assets" value={`${globalStats.otherDominance}%`} change={-0.24} accent="muted" />
        </div>

        <Panel title="Dominance history" description="Last 8 months">
          <Sparkline data={dominanceHistory.map((d) => d.btc)} positive className="h-52" strokeWidth={2} />
        ...
        <Panel title="Interpretation">
          <p className="text-sm text-muted-foreground">
            Rising Bitcoin dominance alongside a rising total market cap indicates capital is entering the market through
            Bitcoin first. Broad altcoin rotation typically begins only once dominance stalls and rolls over.
          </p>
        </Panel>
      </div>
    </AppShell>
